...
class WxPayApi
{
    ...
    public static function unifiedOrder($config, $inputObj, $timeOut = 6)
    {
        ...
        if(!$inputObj->IsOut_trade_noSet()) {
            throw new \think\Exception('缺少统一支付接口必填参数out_trade_no！');
        }else if(!$inputObj->IsBodySet()){
            throw new \think\Exception('缺少统一支付接口必填参数body！');
        }else if(!$inputObj->IsTotal_feeSet()) {
            throw new \think\Exception('缺少统一支付接口必填参数total_fee！');
        }else if(!$inputObj->IsTrade_typeSet()) {
            throw new \think\Exception('缺少统一支付接口必填参数trade_type！');
        }

        //关联参数
        if($inputObj->GetTrade_type() == "JSAPI" && !$inputObj->IsOpenidSet()){
            throw new \think\Exception("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");
        }
        if($inputObj->GetTrade_type() == "NATIVE" && !$inputObj->IsProduct_idSet()){
            throw new \think\Exception("统一支付接口中，缺少必填参数product_id！trade_type为JSAPI时，product_id为必填参数！");
        }
    ...
    public static function orderQuery($config, $inputObj, $timeOut = 6)
    {
        ...
        if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
            throw new \think\Exception("订单查询接口中，out_trade_no、transaction_id至少填一个！");
        }
        $inputObj->SetAppid($config->GetAppId());//公众账号ID
        $inputObj->SetMch_id($config->GetMerchantId());//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
    ...
    public static function closeOrder($config, $inputObj, $timeOut = 6)
    {
        ...
        //检测必填参数
        if(!$inputObj->IsOut_trade_noSet()) {
            throw new \think\Exception("订单查询接口中，out_trade_no必填！");
        }
        $inputObj->SetAppid($config->GetAppId());//公众账号ID
        $inputObj->SetMch_id($config->GetMerchantId());//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
    ...
    public static function refund($config, $inputObj, $timeOut = 6)
    {
        ...
        //检测必填参数
        if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
            throw new \think\Exception("退款申请接口中，out_trade_no、transaction_id至少填一个！");
        }else if(!$inputObj->IsOut_refund_noSet()){
            throw new \think\Exception("退款申请接口中，缺少必填参数out_refund_no！");
        }else if(!$inputObj->IsTotal_feeSet()){
            throw new \think\Exception("退款申请接口中，缺少必填参数total_fee！");
        }else if(!$inputObj->IsRefund_feeSet()){
            throw new \think\Exception("退款申请接口中，缺少必填参数refund_fee！");
        }else if(!$inputObj->IsOp_user_idSet()){
            throw new \think\Exception("退款申请接口中，缺少必填参数op_user_id！");
        }
        $inputObj->SetAppid($config->GetAppId());//公众账号ID
        $inputObj->SetMch_id($config->GetMerchantId());//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
    ...
    public static function refundQuery($config, $inputObj, $timeOut = 6)
    {
        ...
        if(!$inputObj->IsOut_refund_noSet() &&
            ...
            throw new \think\Exception("退款查询接口中，out_refund_no、out_trade_no、transaction_id、refund_id四个参数必填一个！");
        }
        $inputObj->SetAppid($config->GetAppId());//公众账号ID
        $inputObj->SetMch_id($config->GetMerchantId());//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
    ...
    public static function downloadBill($config, $inputObj, $timeOut = 6)
    {
        ...
        if(!$inputObj->IsBill_dateSet()) {
            throw new \think\Exception("对账单接口中，缺少必填参数bill_date！");
        }
        $inputObj->SetAppid($config->GetAppId());//公众账号ID
        $inputObj->SetMch_id($config->GetMerchantId());//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
    ...
    public static function micropay($config, $inputObj, $timeOut = 10)
    {
        ...
        if(!$inputObj->IsBodySet()) {
            throw new \think\Exception("提交被扫支付API接口中，缺少必填参数body！");
        } else if(!$inputObj->IsOut_trade_noSet()) {
            throw new \think\Exception("提交被扫支付API接口中，缺少必填参数out_trade_no！");
        } else if(!$inputObj->IsTotal_feeSet()) {
            throw new \think\Exception("提交被扫支付API接口中，缺少必填参数total_fee！");
        } else if(!$inputObj->IsAuth_codeSet()) {
            throw new \think\Exception("提交被扫支付API接口中，缺少必填参数auth_code！");
        }
    ...
    public static function reverse($config, $inputObj, $timeOut = 6)
    {
        ...
        //检测必填参数
        if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
            throw new \think\Exception("撤销订单API接口中，参数out_trade_no和transaction_id必须填写一个！");
        }
    ...
    public static function report($config, $inputObj, $timeOut = 1)
    {
        ...
        if(!$inputObj->IsInterface_urlSet()) {
            throw new \think\Exception("接口URL，缺少必填参数interface_url！");
        } if(!$inputObj->IsReturn_codeSet()) {
        throw new \think\Exception("返回状态码，缺少必填参数return_code！");
    } if(!$inputObj->IsResult_codeSet()) {
        throw new \think\Exception("业务结果，缺少必填参数result_code！");
    } if(!$inputObj->IsUser_ipSet()) {
        throw new \think\Exception("访问接口IP，缺少必填参数user_ip！");
    } if(!$inputObj->IsExecute_time_Set()) {
        throw new \think\Exception("接口耗时，缺少必填参数execute_time_！");
    }
        $inputObj->SetAppid($config->GetAppId());//公众账号ID
        $inputObj->SetMch_id($config->GetMerchantId());//商户号
    ...
    public static function bizpayurl($config, $inputObj, $timeOut = 6)
    {
        if(!$inputObj->IsProduct_idSet()){
            throw new \think\Exception("生成二维码，缺少必填参数product_id！");
        }
    ...
    public static function shorturl($config, $inputObj, $timeOut = 6)
    {
        ...
        if(!$inputObj->IsLong_urlSet()) {
            throw new \think\Exception("需要转换的URL，签名用原串，传输需URL encode！");
        }
        $inputObj->SetAppid($config->GetAppId());//公众账号ID
        $inputObj->SetMch_id($config->GetMerchantId());//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
